...
```{r}
      ans <- intersect(trees_DV_id$id, trees_EDA_id$id)
      length(ans) #number of such students
      ans #ids
```
...
```{r}
      trees_2 <- inner_join(trees_DV_id, trees_EDA_id, by=c("id"))
      trees_2
```
...
```{r}
      trees_2 %>%
      group_by(id) %>%
      transform(same_question = if_else(anchor.x == anchor.y,TRUE, FALSE)) %>%
      transform(same_answer =if_else(same_question, 
                              if_else(height.x==height.y,TRUE,FALSE),NA)) %>%
      arrange(id, same_question, same_answer)  %>%
      select(id, same_question, same_answer) -> trees_common
      trees_common
...
```
 ...
 \newpage
   ...
   i. *(6 marks)* Using only `trees_all` and `trees_common`,
      ...
      ```
```{r}
      left_join(trees_all,trees_common, by=c("id")) %>%
      arrange(id, class) %>%
      transform(class = if_else(is.na(same_answer), class, 
               if_else(same_question==TRUE & same_answer==TRUE,"BOTH",class)),
               same_question=NULL, same_answer=NULL) %>%
      distinct(id, class,.keep_all = TRUE) -> trees_work
```
...
```{r}
      trees_work %>%
      transform(contradictory = if_else((height >anchor & greater ==FALSE) | 
                           (height <anchor & greater ==TRUE),TRUE,FALSE)) %>%
      filter(contradictory == TRUE)
```
The above students provided answered that are contradictory. They either wrote
that the height is less than anchor value but wrote a height value which is 
greater than anchor or the height is greater than anchor value but wrote a 
height value which is less than the anchor value.
...
```{r}
      trees_work %>%
      ggplot(data = ., mapping = aes(x = height)) +
      geom_histogram(bins = 35, fill = "white", col = "black") +
      geom_density(fill = "grey", alpha = 0.5) + aes(y = ..density..) +
      geom_vline(xintercept = 150, linetype = "dashed", col = "black") + 
      geom_vline(xintercept = 50, linetype = "dashed", col = "black") + 
      geom_vline(xintercept = 115.7,  col = "red") +
      ggtitle("Distribution of estimated heights") + 
